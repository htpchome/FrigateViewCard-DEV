...
test("ha direct mounter transfers HLS listener ownership without restarting playback", async () => {
  await withFakeDocument(async () => {
    await withImmediateTimeout(async () => {
      const hass = {
        states: {
      ...
      const donorVideos = [];
      const receiverVideos = [];
      const donor = createHaDirectMounter({
        getHass: () => hass,
        getPreferredStreamType: () => "hls",
        getStreamMuted: () => true,
        getRotateOverlayActive: () => false,
        isCurrentEngine: (engine) => donorEngine === engine,
        waitForStreamStart: async () => true,
        assignCommittedEngine: (engine) => {
          donorEngine = engine;
        },
        onCommittedMediaReady: (_engine, video) => donorVideos.push(video),
        onCommittedStream: () => {},
        applyResolvedStreamUiState: () => {},
        setLiveNativeControls: () => {},
      });
      const receiver = createHaDirectMounter({
        getHass: () => hass,
        getPreferredStreamType: () => "hls",
        getStreamMuted: () => true,
        getRotateOverlayActive: () => false,
        isCurrentEngine: (engine) => receiverEngine === engine,
        waitForStreamStart: async () => true,
        assignCommittedEngine: (engine) => {
          receiverEngine = engine;
        },
        onCommittedMediaReady: (_engine, video) => receiverVideos.push(video),
        onCommittedStream: () => {},
        applyResolvedStreamUiState: () => {},
        setLiveNativeControls: () => {},
      });
      ...
      await donor.tryMount(slot, null, {
        entity: "camera.front",
        commit: true,
      });
      await flushAsyncWork();
      const stream = donorEngine;
      assert.equal(stream.listenerCount("load"), 1);
      assert.equal(stream.listenerCount("streams"), 1);

      assert.equal(donor.detachForHandoff(stream), true);
      donorEngine = null;
      assert.equal(stream.listenerCount("load"), 0);
      assert.equal(stream.listenerCount("streams"), 0);

      receiverEngine = stream;
      assert.equal(receiver.adoptRetainedEngine(stream), true);
      assert.equal(stream.listenerCount("load"), 1);
      assert.equal(stream.listenerCount("streams"), 1);
      await flushAsyncWork();
      const receiverVideo = { tagName: "VIDEO", id: "receiver-video" };
      stream.video = receiverVideo;
      stream.dispatch("streams");
      await flushAsyncWork();

      assert.equal(donorVideos.length, 1);
      assert.deepEqual(receiverVideos, [stream.firstVideo, receiverVideo]);
      receiver.release(stream);
    });
  });
});

test("ha direct mounter refuses to transfer HLS during WebRTC takeover", async () => {
  const mounter = createHaDirectMounter({
    getHass: () => ({}),
    getPreferredStreamType: () => "webrtc",
    getStreamMuted: () => true,
  ...
  const engine = createFakeStreamElement();
  engine.type = "ha_direct";
  engine.streamType = "hls";
  engine.cancelPendingTakeover = () => {};

  assert.equal(mounter.detachForHandoff(engine), false);
  assert.equal(mounter.adoptRetainedEngine(engine), false);
});
