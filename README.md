...
the PTZ circle control is loaded when the Controls panel first renders, and
...
2. Inside your Home Assistant configuration directory, create this dedicated
   ...
   circle control, dashboard swipe navigation, and HLS companion file from
...
Two-camera groups add per-pane audio selection and focus controls on desktop/tablet, or an A/B source switch on phones. Zoom interaction is limited to the displayed video surface rather than its letterboxed background. Double-click or double-tap toggles zoom, mouse-wheel and pinch gestures zoom around the gesture point, and dragging pans a zoomed image. While the pointer is over displayed media, unmodified `+`/`=` and `-` keys provide keyboard zoom without intercepting browser Control/Command zoom shortcuts or text entry.
