...
| `mobile_view_ha_navbar_dashboard` | boolean | `false` | When off, the bottom-navbar layout follows this card across its internal views. When on, this card owns the layout across the whole dashboard after it loads; only one card per dashboard can own it. |
