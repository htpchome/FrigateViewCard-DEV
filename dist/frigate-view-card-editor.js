    ...
    ${t.map(({value:o,label:n,disabled:s=!1,disabledReason:d=""})=>{const c=fe(o),h=fe(n),p=L(d),g=s===!0;return`<label class="theme-scope-opt card-view-start-opt" data-disabled-guidance="${p}"${g&&p?` title="${p}"`:""}>
      ...
      <div class="setting-content">
        ...
        <div class="settings-more-slot">
          ...
          <span class="editor-choice-chip-body">
            <span class="editor-choice-chip-indicator" aria-hidden="true"></span>
            <span class="editor-choice-chip-text">${D(x[l])}</span>
          </span>
          ...
          <span>${m}</span>
        </label>`).join(""),Jt=l=>l===_.mobileView?"Mobile":l===_.preview?"Preview":l===_.wideView?"Wide View":l===_.cardView?"Card View":"Single View",li=te(this._config,T.desktop).map(l=>({value:l,label:Jt(l)})),di=this._standaloneLandingPageRoutes().map(l=>({value:l,label:Jt(l)})),ci={[f.mobile]:"Mobile",[f.card]:"Card View",[f.previewMobile]:"Preview + Mobile",[f.previewSingle]:"Preview + Single View",[f.single]:"Single View"},hi=Ht(this._config).map(l=>({value:l,label:ci[l]})),Ve=(l,m)=>`<ha-formfield label="${m}">
          <ha-switch data-active-tab="${l}" ${X.has(l)?"":"checked"}></ha-switch>
        </ha-formfield>`,pi=zt.map(l=>{const m=l.key,y=this._themeDefaultHex(m,H),V=le(I[m]),G=le(xe[m]),Ce=V||G||y,Q=J[m]===!0,ze=Q?y:Ce,nt=!Q&&ze!==y;return`
        ...
        </div>`}).join(""),_i=[{value:"light",label:"Light",icon:"mdi:white-balance-sunny"},{value:"dark",label:"Dark",icon:"mdi:weather-night"},{value:"both",label:"Both",icon:"mdi:theme-light-dark"}].map(({value:l,label:m,icon:y})=>{const V=w===l;return`<button
          type="button"
          class="theme-scope-opt ${V?"active":""}"
          data-theme-scope="${l}"
          role="radio"
      ...
      <div class="cam-row" draggable="true" data-row="${m}">
        ...
        <div><div class="cam-name">${D(this._cameraLabel(l))}</div><div class="cam-meta">${D(this._cameraMetaLabel(l))}</div></div>
          ...
          <div class="grid-order-row" draggable="true" data-row="${m}" data-grid-order-entity="${L(l)}">
            ...
            <div class="grid-order-camera-copy">
              <div class="cam-name">${D(this._gridOrderCameraLabel(y))}</div>
              <div class="cam-meta">${D(l)}</div>
            </div>
          ...
          <div class="grid-order-camera-copy">
            <div class="cam-name">${D(this._gridOrderCameraLabel(m))}</div>
            <div class="cam-meta">${D(l)}</div>
          </div>
      ...
      <div class="environment-version-summary">
        <div class="card-version-status" id="card-version-status" data-update-status="unavailable">
          ...
          <div class="card-version-copy">
            <strong>${Oe}</strong>
            <span>Version v${D(Oi)} <span aria-hidden="true">\u2022</span> <span id="card-version-update-status" role="status" aria-live="polite">Update status unavailable</span></span>
      ...
      <div class="text-display-row">
        <ha-input label="Subtitle" name="subtitle" id="subtitle" type="text" value="${L(this._config?.subtitle||Vt)}" placeholder="${L(Vt)}"></ha-input>
      ...
      <div class="section">
        ...
        <div class="chk-row">
          ${Ve("alerts","Alerts")}
          ${Ve("clips","Clips")}
          ${Ve("snapshot","Snapshots")}
          ${Ve("recordings","Recordings")}
          ${Ve("kept","Favorites")}
        </div>
      </div>
      <div class="section">
      ...
      </div>`,B=this._activeSettingsPanelId??null,Di=`
      ...
      </div>`,Vi=`<div id="config-save-reminder" class="config-save-reminder" role="status" aria-live="polite" aria-atomic="true" data-config-save-state="${this._hasConfigDraft===!0?"dirty":"clean"}">
